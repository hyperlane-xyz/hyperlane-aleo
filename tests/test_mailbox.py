...
def get_mapping_value(mapping: str, key: str):
    return get_program_mapping_value("test_hyp_mailbox.aleo", mapping, key)
...
def hook_nonce():
    return int(get_program_mapping_value("test_hyp_hook_manager.aleo", "nonce", "true") or 0)
...
def get_noop_ism():
    ...
    address = get_program_mapping_value("test_hyp_ism_manager.aleo", "ism_addresses", to_aleo_like(nonce, numeric_suffix="32"))
    return address
    
def deploy_noop_hook():
    current_nonce = hook_nonce()
    cwd_transact("execute", "init_noop", cwd="hook_manager")
    address = get_program_mapping_value("test_hyp_hook_manager.aleo", "hook_addresses", to_aleo_like(current_nonce, numeric_suffix="32"))
    return address
...
def test_post_dispatch_custom_hook():
    ...
    merkle_tree_hook = get_program_mapping_value("test_hyp_hook_manager.aleo", "merkle_tree_hooks", hook_address)
    ...
    post_merkle_tree_hook = get_program_mapping_value("test_hyp_hook_manager.aleo", "merkle_tree_hooks", hook_address)
