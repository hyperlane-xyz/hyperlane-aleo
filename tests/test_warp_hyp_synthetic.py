...
PROGRAM = "test_hyp_warp_token_eth.aleo"
LOCAL_DECIMALS = 6
REMOTE_DECIMALS = 18
MAILBOX = {
...
def test_init():
    ...
    if exists:
        return
    program_id = list(b"test_hyp_warp_token_eth.aleo")
...
def test_process_incoming_message_payload_fraction():
    ...
    assert result.get("success"), "Process transaction with fractional payload failed"
    assert int(user_balance_after) - int(user_balance_before) == 999
...
def test_custom_ism():
    ...
    nonce = int(get_program_mapping_value("test_hyp_ism_manager.aleo", "nonce", "true") or 0)
    cwd_transact("execute", "init_domain_routing", cwd="ism_manager")
    routing_ism = get_program_mapping_value("test_hyp_ism_manager.aleo", "ism_addresses", to_aleo_like(nonce, numeric_suffix="32"))
...
def test_transfer_remote():
    ...
    event = get_mapping_value_raw("test_hyp_mailbox.aleo", "dispatch_events", str(nonce) + "u32")
    ...
    balance_after = get_program_mapping_value("token_registry.aleo", "registered_tokens", token_id)
    balance_after = balance_after["supply"] if balance_after else 0

    assert int(balance_before) - int(balance_after) == 1234
...
def test_transfer_custom_hook():
    ...
    igp = get_program_mapping_value("test_hyp_hook_manager.aleo", "igps", IGP)["count"]
    ...
    post_igp = get_program_mapping_value("test_hyp_hook_manager.aleo", "igps", IGP)["count"]
